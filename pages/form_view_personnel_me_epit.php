...
<html lang="en">   
    ...
    <head>
        ...
        <title> Form View Personnel ME</title> 
        <!-- Bootstrap Core CSS -->
           ...
           <div class="panel-heading text-center">
            ...
            <div class="panel-body">
                <div cla    ss="dataTables_wrapper">
                    ...
                    require_once("../php_functions/functions.inc");
                    $class = "table";
                    $headArray = array("SN", "ΑΣΜΑ", "Βαθμός", "Ειδ.", "Επιθετο", "Ονομα",  "Τμήμα/Σμήνος");
                    ...
                    //$sql = "SELECT personnel.*, divisions.*, branches.*, ranks.* FROM personnel, divisions, branches, ranks WHERE personnel.division = divisions.id  AND personnel.branch=branches.id AND ranks.rank=personnel.rank  AND personnel.unit ='{$myUnit}' AND personnel.division ='{$myDivision}' AND (personnel.branch ='{$myBranch1}' OR personnel.branch ='{$myBranch2} OR personnel.branch ='{$myBranch3}') ORDER BY ranks.priority ASC, personnel.asma ASC";
                    //$sql = "SELECT personnel.*, divisions.*, branches.branch as MyBranch, ranks.* FROM personnel, divisions, branches, ranks WHERE personnel.division = divisions.id  AND personnel.branch=branches.id AND ranks.rank=personnel.rank  AND personnel.unit ='{$myUnit}' AND personnel.division ='{$myDivision}' AND personnel.branch ='{$myBranch3}' ORDER BY ranks.priority ASC, personnel.asma ASC";
                    $sql = "SELECT personnel.*, divisions.*, branches.*, ranks.* FROM personnel, divisions, branches, ranks WHERE personnel.division = divisions.id  AND personnel.branch=branches.id AND ranks.rank=personnel.rank  AND personnel.unit ='{$myUnit}' AND personnel.division ='{$myDivision}' AND personnel.branch ='{$myBranch3}' ORDER BY ranks.priority ASC, personnel.asma ASC";
                    $db = new DbMgmt;
                    $res = $db->runQuery($sql);
                    $serial = 1;
                    ...
                    while ($row_personnel = mysqli_fetch_array($res)) {
                        ...
                        echo "<td>" . $row_personnel['branch'] . "</td>";
                        echo "</tr>";
                        $serial = $serial + 1;
                    }
