...
if (isset($_SESSION['error']) && count($_SESSION['error']) > 0) {
    ...
    if (editUser()) {
        ...
        error_log("Problem reseting password: ");
        $_SESSION['error'][] = "Problem reseting password ??!! ";        
        die(header("Location: ../pages/reset_passwd.php"));
    }
}
...
function editUser() {
    ...
    if ($db->runQuery($query)) {
        error_log("Your Password has been reset !");
        return true;            
    } else {
        error_log("Problem reseting your password !!");
