...
if (isset($_SESSION['error']) && count($_SESSION['error']) > 0) {
    die(header("Location: ../pages/form_add_pgr_leave.php"));
} else {
    if (insertLeave()) {
...
function insertLeave() {
    ...
    $location = $db->quote($_POST['location']);
     ...
     $query = "INSERT INTO pgrleave (pl_asma,start_date,num_days,leave_type,pl_location,date_reg) "
            . " VALUES ('{$asma}','{$startdate}','{$numofdays}','{$leave_type}','{$location}',NOW())";
            
                   
    if ($db->runQuery($query)) {
        error_log("new Leave request has been inserted!");
        return true;            
    } else {
        error_log("Problem inserting new Leave request");
