...
$radio_id_update = $db->quote($_POST['radio']);
//$sensor_id_update = 4 ;
...
if (isset($radio_id_update)) {
    if (isset($status_update)) {
        ...
        $sql_update_radio = "UPDATE radio SET status='$status_update', reason='$reason_update', action='$action_update', tbc='$date_update', user_reg='$user_reg', date_reg= NOW() WHERE radio_id='$radio_id_update'";
        $qry_update_radio = $db->runQuery($sql_update_radio);
    }
}


header('Location: ../pages/form_view_radio.php');
?>