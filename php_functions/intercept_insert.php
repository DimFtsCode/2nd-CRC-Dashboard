...
function insertIntercept() {
    ...
    $query = "INSERT INTO `intercept`(`asma`, `cotype`, `mdate`, `stime`, `ltime`, `fcs1`, `numf1`, `typef1`, `sq1`, `fcs2`, `numf2`, `typef2`, `sq2`, `fcs3`, `numf3`, `typef3`, `sq3`, `fcs4`, `numf4`, `typef4`, `sq4`, `extcard`, `area`, `alt`, `numint`, `intype`, `numint2`, `intype2`, `freq`, `radio`, `post`, `aj`, `ajnet`, `crypto`, `mids`, `comq`, `eng`, `iff`, `reason`, `reason2`, `remark`, `debrief`, `briefing`, `kio`, `timing`, `user_reg`, `date_reg`)"
           . " VALUES ('{$asma}', '{$cotype}', '{$mdate}', '{$stime}', '{$ltime}', '{$fcs1}', '{$numf1}', '{$typef1}', '{$sq1}', '{$fcs2}', '{$numf2}', '{$typef2}', '{$sq2}', '{$fcs3}', '{$numf3}', '{$typef3}', '{$sq3}', '{$fcs4}', '{$numf4}', '{$typef4}', '{$sq4}', '{$extcard}', '{$area}', '{$alt}', '{$numint}', '{$intype}', '{$numint2}', '{$intype2}', '{$freq}', '{$radio}', '{$post}', '{$aj}', '{$ajnet}', '{$crypto}', '{$mids}', '{$comq}', '{$eng}', '{$iff}', '{$reason}', '{$reason2}', '{$remark}', '{$debrief}', '{$briefing}', '{$kio}', '{$timing}', '{$user_reg}', NOW())";
       
    if ($db->runQuery($query)) {
        $_SESSION['error'][] = " Intercept successfully Added to DB !! "; 
        return true;          
    } else {
        $_SESSION['error'][] = " Problem inserting intercept to DB !! ";        
        return false;
    }    
}
...
if (isset($_SESSION['error']) && count($_SESSION['error']) > 0) {
    ...
    if (insertIntercept()) {
        ...
        error_log("Problem inserting new interception: ");
        $_SESSION['error'][] = "Problem inserting new Interception  ??!! ";        
        die(header("Location: ../pages/form_add_intercept.php"));
    }
}
?>
