...
if (isset($_SESSION['error']) && count($_SESSION['error']) > 0) {
    ...
    if (editLog()) {
        ...
        $_SESSION['error'][] = "Problem editing new log record !! ";            
        die(header("Location: ../pages/form_edit_logbook.php"));
    }
}
...
//                    . "VALUES ('{$description}', '{$mdate}', '{$mtime}', '{$load}', '{$remark}', '{$user_reg}',NOW())";
           ...
           // $query = "UPDATE logbook SET description='$description', mdate='$mdate', mtime='$mtime', load='$load', remark='$remark', user_reg='$user_reg', date_reg= NOW()";
        
             $query = "UPDATE `logbook` SET `description`='$description', `mdate`='$mdate', `mtime`='$mtime', `load`='$load', `remark`='$remark', `user_reg`='$user_reg', `date_reg`=NOW()WHERE `log_id`='$log_id' ";
                    
 if ($db->runQuery($query)) {
        error_log(" Log has been added successfully !");
        return true;            
    } else {
        error_log(" Problem on editing new log record  ! ");
