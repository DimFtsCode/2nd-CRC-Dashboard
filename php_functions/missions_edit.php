...
if (!isset($_POST['submit'])) {
    die(header("Location: ../pages/form_edit_missions.php"));
}
...
$sql_update = "UPDATE missions SET asset='$asset', callsign='$callsign', track='$track', mission='$mission', track2='$track2', user_reg='$user_reg', date_reg= NOW() "
            . ", result='$result', area='$area', mdate='$mdate', mtime='$mtime', scope='$scope', remark='$remark' WHERE mis_id='$mis_id' ";
            
 if ($db->runQuery($sql_update)) {
        error_log(" Mission has been edited successfully !");
        return true;            
    } else {
        error_log(" Problem on editing Mission ! ");
